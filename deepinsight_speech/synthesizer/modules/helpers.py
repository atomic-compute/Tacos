...
from tensorflow_addons.seq2seq import SampleEmbeddingSampler as Helper
...
class TrainingHelper(Helper):
    ...
    def __init__(self, targets, output_dim, r, n_feed_frame=1):
        ...
        self.sample_ids_dtype = tf.int32
        self.sample_ids_shape = tf.TensorShape([])
...
class TransformerTrainingHelper(Helper):
    ...
    def __init__(self, targets, output_dim, r, n_feed_frame=1, teacher_forcing=False):
        assert n_feed_frame <= r
        t_shape = tf.shape(targets)
        self.teacher_forcing = teacher_forcing
        self.batch_size = t_shape[0]
...
class ValidationMgcLf0Helper(Helper):
    ...
    def next_inputs(self, time, outputs, state, sample_ids, name=None):
        mgc_output, lf0_output, done = outputs
        finished = (time + 1 >= self.num_steps)
        next_mgc_inputs = self._mgc_targets[
            :, time, -self._mgc_output_dim * self.n_feed_frame:
        ] if self.teacher_forcing else mgc_output[
            :, -self._mgc_output_dim * self.n_feed_frame:
        ]
        next_mgc_inputs.set_shape([mgc_output.get_shape()[0].value, self._mgc_output_dim * self.n_feed_frame])
        lf0_output = tf.nn.softmax(tf.reshape(lf0_output, shape=[-1, self.r, self._lf0_output_dim]), axis=-1)
        next_lf0_inputs = self._lf0_targets[
            :, time, -self._lf0_output_dim * self.n_feed_frame:
        ] if self.teacher_forcing else tf.reshape(
