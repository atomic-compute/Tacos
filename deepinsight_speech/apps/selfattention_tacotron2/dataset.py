...
class CharactorMelDataset(AbstractDataset):
    ...
    def __init__(
        ...
        self.silence_mel_level_db = silence_mel_level_db
        self.average_mel_level_db = average_mel_level_db
        self.stddev_mel_level_db = stddev_mel_level_db
    ...
    def convert(self, mel_gts, mel_lengths):

        mel_normalized = (
            (mel_gts - np.array(self.average_mel_level_db, dtype=np.float32)) /
            np.array(self.stddev_mel_level_db, dtype=np.float32)
        )

        mel_with_silence = tf.pad(
            mel_normalized,
            paddings=[[self.outputs_per_step, self.outputs_per_step], [0, 0]],
            constant_values=self.silence_mel_level_db
        )

        # +2r for head and tail silence
        target_length = mel_lengths + 2 * self.outputs_per_step
        padded_target_length = (target_length // self.outputs_per_step + 1) * self.outputs_per_step
        ...
        def padding_function(t):
            ...
            padding_shape = tf.sparse_tensor_to_dense(tf.SparseTensor(
                indices=[(0, 1)],
                values=tf.expand_dims(tail_padding, axis=0), dense_shape=(2, 2)
            ))
            return lambda: tf.pad(t, paddings=padding_shape, constant_values=self.silence_mel_level_db)

        no_padding_condition = tf.equal(tf.to_int64(0), target_length % self.outputs_per_step)

        mel_gts = tf.cond(
            no_padding_condition,
            lambda: mel_with_silence, padding_function(mel_with_silence)
        )

        mel_gts.set_shape((None, self.num_mels))

        padded_target_length = tf.cond(
            no_padding_condition, lambda: target_length, lambda: padded_target_length)

        done = tf.concat([
            tf.zeros(padded_target_length // self.outputs_per_step - 1, dtype=tf.float32),
            tf.ones(1, dtype=tf.float32)
        ], axis=0)

        return mel_gts, done, padded_target_length
    ...
    def _load_data(self, items):
        ...
        mel, done, padded_target_length = self.convert(mel, mel_length)
        items = {
