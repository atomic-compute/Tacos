"""Perform preprocessing and raw feature extraction for VCTK dataset."""
...
@dataclass
class VCTKProcessor(BaseProcessor):
    ...
    def create_items(self):
        if self.data_dir:
            fieldnames = ("wave_file", "text", "speaker_name", "speaker_age", "speaker_gender", "speaker_accent")
            with open(os.path.join(self.data_dir, self.train_f_name), encoding='utf-8') as f:
                next(f)
                reader = csv.DictReader(f, fieldnames)
                self.items = [self.split_line(self.data_dir, row) for row in reader]

    def split_line(self, data_dir, row):
        wave_file = row["wave_file"]
        text_norm = row["text"]
