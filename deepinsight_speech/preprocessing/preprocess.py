...
try:
    ...
    sys.path.append(os.path.abspath(
        Path(Path(__file__).parts[0], os.pardir)))
...
from deepinsight_speech.preprocessing.vctk import VCTKProcessor
from deepinsight_speech.preprocessing.librispeech import LibriSpeechProcessor
...
from deepinsight_speech.preprocessing.librispeech import LIBRISPEECH_SYMBOLS
from deepinsight_speech.preprocessing.vctk import VCTK_SYMBOLS

from tensorflow_tts.utils import remove_outlier
from dataclasses import dataclass
from deepinsight_speech.preprocessing.utils import save_features_to_file

os.environ["CUDA_VISIBLE_DEVICES"] = ""


def parse_and_config(args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="ljspeech",
        choices=["ljspeech", "kss", "libritts", "baker", "thorsten", "ljspeechu", "synpaflex", "vctk", "librispeech"],
...
def ph_based_trim(
    ...
    audio: np.array,
    hop_size: int,
) -> typing.Tuple[bool, np.array, np.array]:
    """
    Args:
...
def gen_audio_features(item, config):
    ...
    if config["trim_silence"]:
        if "trim_mfa" in config and config["trim_mfa"]:
            ...
            # very short files can get trimmed fully if mfa didnt extract any
            # tokens LibriTTS maybe take only longer files?
            if (audio.__len__() < 1):
...
def preprocess(args=None):
    ...
    dataset_symbol = {
        ...
        "vctk": VCTK_SYMBOLS,
        "librispeech": LIBRISPEECH_SYMBOLS
    }
    ...
    if config["dataset"] in ("libritts", "librispeech"):
...
if __name__ == "__main__":
    ...
    try:
        fname = sys.argv[1]
        func = getattr(inspect.getmodule(sys.modules[__name__]), fname)
    except:
